...
# near_ten(19) -> True
def near_ten(n):
  return True if (n + 2) % 10 == 0 or (n - 2) % 10 == 0 else False
...
def count_code(s):
  if all([ s.find('coxe'), s.find('coze'), s.find('code') ]):
    return s.count('code') + s.count('coxe') + s.count('coze') + s.count('code')
  return s.count('code')
...
def end_other(a, b):
  textA = a.lower()
  textB = b.lower()
  return textA.endswith(textB) or textB.endswith(textA)
...
# sum13([13, 1, 2, 3, 4]) -> 0
def sum13(nums):
  return sum(nums) if not 13 in nums else sum(nums[:nums.index(13)])
...
def has22(nums):
  num = str(nums)
  if num.find('2') and 2 in nums[nums.index(2) + 1:] and num.count('2') == 2: return True 
  # if 2 in nums 
  #   return True
  # return False
