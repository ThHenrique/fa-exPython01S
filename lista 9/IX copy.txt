...
def first_last6(nums): #
  return 6 in nums[:1] or 6 in nums[-1:]
...
def same_first_last(nums):
  return True if len(nums) >= 1 and nums[:1] == nums[-1:] else False
...
def common_end(a, b):
  return True if a[:1] == b[:1] or a[-1:] == b[-1:] else False
...
def squirrel_play(temp, is_summer):
  return True if (temp >= 60 and temp <= (90 if not is_summer else 100)) else False
